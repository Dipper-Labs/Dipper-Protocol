...
$(go list ./... | grep -v 'Dipper-Protocol/server' | grep -v '/simulation' | grep -v mock | grep -v 'Dipper-Protocol/tests' | grep -v 'crypto' | grep -v '/simapp' | grep -v '/app/genesis')
...
do
  filename=$(echo ${filename} | sed 's/^./github.com\/Dipper-Labs\/Dipper-Protocol/g')
