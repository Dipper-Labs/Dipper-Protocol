...
type DIPAppWithGenAccounts struct {
	*baseapp.BaseApp
	GenesisAccounts []authexported.Account
}

type testInput struct {
	mApp     *DIPAppWithGenAccounts
	keeper   gov.Keeper
	sk       staking.Keeper
...
func getProtocolV0(t *testing.T, app *DIPAppWithGenAccounts) *v0.ProtocolV0 {
...
func NewDIPApp(t *testing.T) *DIPAppWithGenAccounts {
	logger := log.NewNopLogger()
	db := dbm.NewMemDB()
	baseApp := baseapp.NewBaseApp("dipmock", logger, db)
	...
	return &DIPAppWithGenAccounts{BaseApp: baseApp}
}

func setGenesis(app *DIPAppWithGenAccounts, cdc *codec.Codec, accs []auth.Account, genState gov.GenesisState) error {
...
func setTotalSupply(t *testing.T, app *DIPAppWithGenAccounts, ctx sdk.Context, accAmt sdk.Int, totalAccounts int) {
...
func initGenAccount(t *testing.T, ctx sdk.Context, app *DIPAppWithGenAccounts) {
	p0 := getProtocolV0(t, app)
	accAmt := sdk.NewInt(0)
